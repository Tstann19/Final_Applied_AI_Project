...
from ui_helpers import inject_css, hint_html, number_hint_html, banner, guess_row_html, IDLE_CARD

st.set_page_config(page_title="Glitchy Guesser", page_icon="🎮")
inject_css()

st.title("🎮 Game Glitch Investigator")
st.caption("An AI-powered guessing game. Something is off.")

tab1, tab2 = st.tabs(["🔢 Number Guesser", "🎮 Pokémon Guesser"])
...
with tab1:
    ...
    st.progress(
        ...
        text=f"{st.session_state.attempts} of {attempt_limit} attempts used",
    )
    ...
    if st.session_state.status == "won":
        st.markdown(
            banner(f"You won! The secret was {st.session_state.secret}. Score: {st.session_state.score}", win=True),
            unsafe_allow_html=True,
        )
    elif st.session_state.status == "lost":
        st.markdown(
            banner(f"Out of attempts! The secret was {st.session_state.secret}. Score: {st.session_state.score}", win=False),
            unsafe_allow_html=True,
        )
    else:
        if submit:
            ...
            if not ok:
                ...
                if show_hint and outcome != "Win":
                    st.markdown(number_hint_html(message), unsafe_allow_html=True)
...
with tab2:
    ...
    st.caption("A secret Pokémon has been chosen. Decode the AI-generated riddles to identify it!")
    ...
    if st.session_state.poke_status == "idle":
        st.markdown(IDLE_CARD, unsafe_allow_html=True)
    else:
        ...
        st.progress(
            min(attempts_used / attempt_limit_pk, 1.0),
            text=f"{attempts_used} of {attempt_limit_pk} attempts used",
        )
        ...
        st.markdown(
            "".join(hint_html(i, len(hints), hints[i]) for i in range(num_revealed)),
            unsafe_allow_html=True,
        )
        ...
        if st.session_state.poke_status == "won":
            st.balloons()
            st.markdown(
                banner(f"Correct! It was {pokemon['display_name']} (#{pokemon['id']})! Score: {st.session_state.poke_score}", win=True),
                unsafe_allow_html=True,
            )
        elif st.session_state.poke_status == "lost":
            st.markdown(
                banner(f"No guesses left! The Pokémon was {pokemon['display_name']} (#{pokemon['id']}).", win=False),
                unsafe_allow_html=True,
            )
        else:
            ...
            if st.button("Submit Guess 🎯", key="poke_submit_guess"):
                if not poke_guess.strip():
                    st.warning("Type a Pokémon name before submitting!")
                else:
        ...
        if st.session_state.poke_history:
            with st.expander(f"Guess history ({len(st.session_state.poke_history)})"):
                for i, g in enumerate(st.session_state.poke_history, 1):
                    st.markdown(
                        guess_row_html(i, g, check_pokemon_guess(g, pokemon["name"])),
                        unsafe_allow_html=True,
                    )

    st.divider()
    st.caption("Pokémon data from PokeAPI · Riddles powered by Gemini AI")
